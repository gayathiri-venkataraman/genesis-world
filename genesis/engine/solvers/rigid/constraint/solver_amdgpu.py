...
@qd.func
def _func_solve_iter_post_linesearch(
    i_b,
    ...
    """Post-linesearch half of func_solve_iter (alpha already applied)."""
...
def _kernel_solve_iter_post_linesearch_amdgpu(
    ...
    for i_b in range(_B):
        if constraint_state.n_constraints[i_b] > 0 and constraint_state.improved[i_b]:
            _func_solve_iter_post_linesearch(
                i_b,
                dyn_state,
                constraint_state,
                dyn_info,
...
def func_solve_body_split_amdgpu(
    dyn_state, constraint_state, dyn_info, rigid_info, rigid_config, _n_iterations,
):
    solver.func_solve_init(dyn_state, constraint_state, dyn_info, rigid_info, rigid_config, is_decomposed=False)
    # _n_iterations is a Python-native int (avoids GPU sync that
    # rigid_info.iterations[None] would force).
...
def _decomposed_amdgpu_is_compatible(*args, **kwargs):
    ...
    # ConstraintSolver.solve), so rigid_config arrives as args[4].
    ...
    cfg = kwargs.get("rigid_config", args[4] if len(args) >= 5 else None)
    if cfg is None:
        return False
    if cfg.sparse_solve:
        return False
...
def _func_ls_init_p0_wc(
    ...
    n_entities = rigid_config.n_entities_
    ne = constraint_state.n_constraints_equality[i_b]
    nef = ne + constraint_state.n_constraints_frictionloss[i_b]
    n_con = constraint_state.n_constraints[i_b]
...
def func_linesearch_batch_wavecoop(
    ...
    """
    snorm, gtol = _func_snorm_wc(i_b, tid, constraint_state, rigid_info)
    ls_it = gs.qd_int(0)
    ...
    if not done:
        ...
        if qd.abs(p1_deriv_0) < gtol:
            ...
            phase2_break = False
            while p1_deriv_0 * direction <= -gtol and ls_it < rigid_info.ls_iterations[None]:
                p2_alpha = p1_alpha
                p2_cost = p1_cost
                p2_deriv_0 = p1_deriv_0
                p2_deriv_1 = p1_deriv_1
            ...
            if not phase2_break:
                if ls_it >= rigid_info.ls_iterations[None]:
                    ls_result = 3
                    res_alpha = p1_alpha
                    done = True
                ...
                if not done:
                    ...
                    p3_done = False
                    while ls_it < rigid_info.ls_iterations[None]:
...
def _kernel_solve_body_wavecoop_amdgpu(
    ...
    for i in range(_B * BLOCK_DIM):
        ...
        for _it in range(rigid_info.iterations[None]):
            n_con = constraint_state.n_constraints[i_b]
            ...
            if qd.abs(alpha_val) < rigid_info.EPS[None]:
                if tid == 0:
                    constraint_state.improved[i_b] = False
                ...
                for i_e in range(qd.static(rigid_config.n_entities_)):
                    if rigid_info.mass_mat_mask[i_e, i_b]:
                        ...
                        for pp in range(e_n):
                            ...
                            k = e_ds + tid
                            while k < p:
                                msolve[k] = msolve[k] - rigid_info.mass_mat_L[p, k, i_b] * wp
                                k = k + BLOCK_DIM
                            qd.simt.block.sync()
                        ...
                        i_d = e_ds + tid
                        while i_d < e_de:
                            msolve[i_d] = msolve[i_d] * rigid_info.mass_mat_D_inv[i_d, i_b]
                            i_d = i_d + BLOCK_DIM
                        qd.simt.block.sync()
                        ...
                        for pp in range(e_n):
                            ...
                            k = p + 1 + tid
                            while k < e_de:
                                msolve[k] = msolve[k] - rigid_info.mass_mat_L[k, p, i_b] * wp
                                k = k + BLOCK_DIM
                            qd.simt.block.sync()
            ...
            tol_scaled = (
                rigid_info.meaninertia[i_b] * qd.max(1, N_DOFS)
            ) * rigid_info.tolerance[None]
            ...
            if improved6:
                ...
                if tid == 0:
                    ...
                    cg_beta = qd.max(t_beta / qd.max(rigid_info.EPS[None], t_pgpm), 0.0)
...
def func_solve_body_wavecoop_amdgpu(
    ...
    # is unused here -- the kernel reads rigid_info.iterations[None].
...
def func_linesearch_batch_tiled_wc(
    ...
    """
    snorm, gtol = _func_snorm_twc(i_b, tid, constraint_state, rigid_info)
    ls_it = gs.qd_int(0)
    ls_result = gs.qd_int(0)
    res_alpha = gs.qd_float(0.0)
    done = False
...
def _kernel_solve_body_tiled_wc_amdgpu(
    ...
    for i in range(N_BLOCKS * BLOCK_DIM):
        ...
        for _it in range(rigid_info.iterations[None]):
            ...
            tiny_alpha = qd.abs(alpha_val) < rigid_info.EPS[None]
            ...
            if qd.static(rigid_config.solver_type == gs.constraint_solver.CG):
                for i_e in range(qd.static(rigid_config.n_entities_)):
                    ...
                    do_e = False
                    if is_active_env:
                        do_e = bool(rigid_info.mass_mat_mask[i_e, i_b])
                        ...
                        if do_e:
                            ...
                            while k < p:
                                msolve_t[env_in_block, k] = (
                                    msolve_t[env_in_block, k] - rigid_info.mass_mat_L[p, k, i_b] * wp
                                )
                                k = k + COOP
                        qd.simt.block.sync()
                    ...
                    if do_e:
                        ...
                        while i_d < e_de:
                            msolve_t[env_in_block, i_d] = (
                                msolve_t[env_in_block, i_d] * rigid_info.mass_mat_D_inv[i_d, i_b]
                            )
                            i_d = i_d + COOP
                    qd.simt.block.sync()
                    ...
                    for pp in range(e_n):
                        ...
                        if do_e:
                            ...
                            while k < e_de:
                                msolve_t[env_in_block, k] = (
                                    msolve_t[env_in_block, k] - rigid_info.mass_mat_L[k, p, i_b] * wp
                                )
                                k = k + COOP
                        qd.simt.block.sync()
